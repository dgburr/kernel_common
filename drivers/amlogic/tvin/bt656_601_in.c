...
//            pr_dbg("there is no vdin interrupt, and error in bt656in, ccir656_status = %x, vdin_com_status %x. \n" , ccir656_status,vdin_com_status);
...
//        pr_error("bt656in decoder is not started\n");
    ...
    if(vdin_devp_bt656->para.flag & TVIN_PARM_FLAG_CAP)
    {
        ...
        {
            if(am656in_dec_info.rd_canvas_index == 0)
                  canvas_id = am656in_dec_info.canvas_total_count - 1;
            else
                  canvas_id = am656in_dec_info.rd_canvas_index - 1;
        }
#else
        canvas_id = am656in_dec_info.rd_canvas_index;   //kernel & user don't use the same memory
#endif
