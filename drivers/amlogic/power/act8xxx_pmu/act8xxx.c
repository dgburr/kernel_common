...
//export this interface for other driver
void act8xxx_set_outn_poweroff(int outn)
{	
	u8 val;	
    u32 out_on_reg[] = {ACT8xxx_REG1_ADDR + 2,
                        ACT8xxx_REG2_ADDR + 2, 
                        ACT8xxx_REG3_ADDR + 2,
                        ACT8xxx_REG4_ADDR + 1, 
                        ACT8xxx_REG5_ADDR + 1, 
	...
	if(this_client != NULL)
	{
        if ((outn >= ACT8xxx_REG1) && (outn <= ACT8xxx_REG7))
	    {
	        reg = out_on_reg[outn - ACT8xxx_REG1];
	        act8xxx_read_i2c(this_client, reg, &val);

            //set ON bit to 1
	        val |= (1<<7);
	        act8xxx_write_i2c(this_client, reg, &val);
            //set ON bit to 0
	        val &= ~(1<<7);
	        act8xxx_write_i2c(this_client, reg, &val);
	    }
        ...
        {
            pr_err("act8xxx: Wrong REG number!%d REG number in [1:7]\n", outn);
        }
...
EXPORT_SYMBOL(act8xxx_set_outn_poweroff);
...
ssize_t bat_refresh(struct class *class, struct class_attribute *attr, char *buf)
{
    int re = 0;
    if(buf[0] == '1')
        re = 1;
...
ssize_t act8xxx_voltage_handle(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
...
static struct class_attribute act8xxx_class_attrs[] = {
	...
	__ATTR(bat_refresh, S_IRUGO | S_IWUSR, NULL, bat_refresh),
	__ATTR_NULL
};
...
inline static int measure_capacity_advanced(void)	
{
	...
	if(act8942_opts->rvp&&act8xxx_dev->capacity != -1 && !refresh)
	{
        if (current_status) {
...
#ifdef CONFIG_PMU_ACT8942	
static void act8xxx_early_suspend(struct early_suspend *h)
{
	act8942_opts->set_charge_current(1);
	pr_info("fast charger on early_suspend\n\n");    
}

static void act8xxx_late_resume(struct early_suspend *h)
{
...
static struct early_suspend act8xxx_suspend = {
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
	.suspend = act8xxx_early_suspend,
	.resume = act8xxx_late_resume,
	.param = NULL,
};
#endif

static int act8xxx_i2c_resume(struct i2c_client *client)
{   int i;
    for(i = 0;i< SAMPLE_SIZE;i++){
        capacity_sample_array[i] = -1;
    }
    capacity_sample_pointer = 0;
    refresh = 1;
}
				 ...
				 const struct i2c_device_id *id)
{
	...
	act8xxx_dev->polling_timer.data = (unsigned long)act8xxx_dev;
	add_timer(&(act8xxx_dev->polling_timer));
#endif

#ifdef CONFIG_HAS_EARLYSUSPEND
	register_early_suspend(&act8xxx_suspend);
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&act8xxx_suspend);
#endif
	return 0;
}
...
static long act8xxx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	int size, i;
	...
	if(_IOC_DIR(cmd) == _IOC_READ)
	{
		...
		if(size)
		{
			...
			ret = copy_to_user((void __user *)arg, msgs, (sizeof(act8xxx_i2c_msg_t)*size));
			kfree(msgs);
			return 0;
		}
		else
