...
//            hdmi_wr_reg(0x010, 0x2);
            switch(vic)
            {
                ...
                case HDMI_576i50_16x9:
                    ...
                    hdmi_wr_reg(TX_HDMI_PHY_CONFIG6, (0x3 << HDMI_SWING_CTL) | 
                                                     (0x0 << HDMI_RTERM_CTL) );
                    break;
                case HDMI_720p60:
                case HDMI_720p50:
                ...
                case HDMI_1080p50:
                    ...
                    hdmi_wr_reg(TX_HDMI_PHY_CONFIG5, (0x7 << HDMI_VCM_CTL) | 
                                                     (0x7 << HDMI_PREFCTL));         //0x15 Slew
                    hdmi_wr_reg(TX_HDMI_PHY_CONFIG6, (0x4 << HDMI_SWING_CTL) | 
                                                     (0x0 << HDMI_RTERM_CTL) );
                    break;
                default:
                    hdmi_wr_reg(TX_HDMI_PHY_CONFIG0, 0xfe << HDMI_COMMON_b7_b0);    //0x10
...
static void enable_audio_spdif(void)
{
    ...
    printk("Enable audio spdif to HDMI\n");
        ...
        aml_write_reg32(P_AIU_958_MISC, 0x204a ); // // Program the IEC958 Module in the AIU
        aml_write_reg32(P_AIU_958_FORCE_LEFT, 0x0000 );
        aml_write_reg32(P_AIU_958_CTRL, 0x0240 );
    ...
    /* enable audio*/        
        ...
        aml_write_reg32(P_AIU_CLK_CTRL,        aml_read_reg32(P_AIU_CLK_CTRL) | 2); // enable iec958 clock which is audio_master_clk
        aml_write_reg32(P_AIU_958_BPF, 0x0100 ); // Set the PCM frame size to 256 bytes
        aml_write_reg32(P_AIU_958_DCU_FF_CTRL, 0x0001 );
        
        aml_write_reg32(P_AIU_I2S_MISC, aml_read_reg32(P_AIU_I2S_MISC)|0x18); //i2s_to_958 directly

#ifdef AML_A3
    //hdmi 958 clk  = amclk/128/2;
        aml_write_reg32(P_AIU_CLK_CTRL, (aml_read_reg32(P_AIU_CLK_CTRL)&(~(0x3<<4))&(~(1<<12)))|(0x1<<4));
#endif
}
...
static void enable_audio_i2s(void)
{
    ...
    aml_write_reg32(P_AIU_CLK_CTRL,        aml_read_reg32(P_AIU_CLK_CTRL) | 3); // enable iec958 clock which is audio_master_clk
    aml_write_reg32(P_AIU_CLK_CTRL, (aml_read_reg32(P_AIU_CLK_CTRL)&(~(0x3 << 4)))|(1<<4));
#ifdef AML_A3
    //hdmi 958 clk  = amclk/128/2;
        aml_write_reg32(PAIU_CLK_CTRL, (aml_read_reg32(P_AIU_CLK_CTRL)&(~(0x3<<4))&(~(1<<12)))|(0x1<<4));
#endif
...
//    }
        ...
        if(argv){
            hdmi_wr_reg(TX_HDMI_PHY_CONFIG1, hdmi_rd_reg(TX_HDMI_PHY_CONFIG1)|0xf0);
        }
        else{
            hdmi_wr_reg(TX_HDMI_PHY_CONFIG1, hdmi_rd_reg(TX_HDMI_PHY_CONFIG1)&(~0xf0));
        }
...
{
    printk("hdmitx_device addr:%x", hdmitx_device);
...
//\\ TODO
    ...
    printk("HDMI REG0x10:0x%x", hdmi_rd_reg(0x10));
    HDMI_DEBUG();
